...
Ответ строго в JSON, БЕЗ markdown, БЕЗ преамбулы:
{
  ...
  "fit_reason": "fit_reason": "ОДНО короткое предложение (до 15 слов) с сутью роли: что за должность, в каком контексте, ключевая специфика вакансии. Это НЕ оценка совпадения — это описание роли. Пример: 'Senior разработка бэкенда на Kotlin в финтех-компании, полностью удалённо' или 'Продюсер спецпроектов в EdTech с фокусом на B2B-коллаборации'. Пиши так, чтобы юзер сразу понял, о какой роли речь.",

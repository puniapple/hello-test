...
КРИТИЧНО: Твой ответ должен быть ТОЛЬКО валидным JSON-объектом. Не добавляй ничего до, ничего после. Никаких markdown-обёрток типа ```json ... ```. Никаких объяснений или комментариев после закрывающей скобки }.

Всю аналитику и рассуждения помещай ВНУТРИ полей red_flags и fit_reason. Не пиши "Краткое объяснение:", "Итого:" или подобные фразы после JSON.

Формат ответа (начинается с { и заканчивается на }, ничего больше):
{
  "score": число от 0 до 10 (можно с десятыми, например 7.5),
  "fit_reason": "ОДНО короткое предложение (до 15 слов) с сутью роли",
  "red_flags": ["короткие", "формулировки"],
...
class VacancyMatcher:
    ...
    def _parse_response(self, text: str) -> MatchResult | None:
        ...
        # Извлекаем первый валидный JSON-объект из ответа Haiku.
        # Модель иногда оборачивает в ```json ... ``` фенсы и/или пишет
        # постскриптум типа "Краткое объяснение:" после закрывающей }.
    ...
    def _extract_first_json_object(self, text: str) -> str | None:
        ...
        Находит первую { и её парную закрывающую }, учитывая вложенность
        и строки со скобками внутри значений. Возвращает подстроку или None.
        """
        start = text.find("{")
        if start == -1:
            return None

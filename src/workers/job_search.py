            ...
            if user.empty_streak_days >= 3 and user.empty_notice_sent_at is None:
                try:
                    await bot.send_message(
                        chat_id=user.telegram_id,
                        text=(
                            "Уже 3 дня подряд не могу подобрать тебе ничего подходящего.\n\n"
                            "Часто дело в том, что профиль сформулирован слишком узко "
                            "или где-то есть противоречия. Могу помочь — обнови его через /edit_profile.\n\n"
                            "Если поиск сильно поменялся или в жизни изменились планы — "
                            "тоже возвращайся, начнём заново."
                        ),
                    )
